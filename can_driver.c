 ...
 */
static enum can_error can_wait_for_clock_active(uint32_t base,
                                                uint32_t timeout);
...
can_configure_tx_fifo_queues(uint32_t base, const struct can_config *cfg);
static enum can_error
...
enum can_error can_init(const struct can_config *cfg) {
  ...
  /* Step 10: Enable Interrupts in IRC */
...
enum can_error can_deinit(uint32_t base_addr) {
  ...
  while (timeout > 0U) {
    ...
    if (!(reg_val & CAN_FQ_STS0_BUSY_MASK))
      break;
    timeout--;
  }
...
/*
 * can_set_bit_timing - Configure bit timing
 * @base_addr: CAN controller base address
 * @nominal: Nominal timing configuration
 * @data: Data phase timing (CAN FD), may be NULL
 * @xl: XL phase timing (CAN XL), may be NULL
 *
 * Return: enum can_error code
 */
enum can_error can_set_bit_timing(uint32_t base_addr,
  ...
  if (data) {
    dbtp_val = CAN_BUILD_DBTP(data->tdco, data->tseg1, data->tseg2, data->sjw);
 ...
 * @base_addr: CAN controller base address
 * @func_mask: Functional interrupt mask
 * @err_mask: Error interrupt mask
 * @safety_mask: Safety interrupt mask
 *
 * Return: enum can_error code
 */
...
/*
 * can_interrupt_clear - Clear interrupt flags
 * @base_addr: CAN controller base address
 * @func_mask: Functional interrupt mask to clear
 * @err_mask: Error interrupt mask to clear
 * @safety_mask: Safety interrupt mask to clear
 *
 * Return: enum can_error code
 */
...
/*
 * can_interrupt_get_raw_status - Get raw interrupt status
 * @base_addr: CAN controller base address
 * @func_status: Pointer to functional status (may be NULL)
 * @err_status: Pointer to error status (may be NULL)
 * @safety_status: Pointer to safety status (may be NULL)
 *
 * Return: enum can_error code
 */
...
/*
 * can_tx_fifo_start - Start TX FIFO queue
 * @base_addr: CAN controller base address
...
/*
 * can_rx_fifo_start - Start RX FIFO queue
 * @base_addr: CAN controller base address
 * @queue_idx: Queue index (0-7)
 * @config: Queue configuration
 *
...
static enum can_error can_configure_mh_global(uint32_t base,
  ...
  mh_cfg_val |=
      (0x7U << CAN_MH_CFG_MAX_RETRANS_POS) & CAN_MH_CFG_MAX_RETRANS_MASK;
...
static enum can_error can_configure_bit_timing(uint32_t base,
  ...
  if (cfg->protocol == CAN_PROTOCOL_FD || cfg->protocol == CAN_PROTOCOL_XL) {
    dbtp_val = CAN_BUILD_DBTP(cfg->data_timing.tdco, cfg->data_timing.tseg1,
...
static void can_smem_copy_data(uint32_t dst_addr, const uint8_t *src,
  ...
  for (i = 0U; i < (len / 4U); i++) {
    word = ((uint32_t)src[offset + 0U]) | ((uint32_t)src[offset + 1U] << 8U) |
...
enum can_error can_tx_fifo_push_ext(uint32_t base_addr, uint8_t fifo_id,
  ...
  if (xl) {
    ...
    desc.t0 =
        CAN_BUILD_T0_EXT((id >> 18U) & 0x7FFU, id & 0x3FFFFU, fd ? 1U : 0U);
  } else {
...
enum can_error can_tx_abort(uint32_t base_addr, uint8_t fifo_id) {
  ...
  while (timeout > 0U) {
    reg_val = CAN_READ_REG(base_addr, CAN_MH_TX_FQ_STS0_OFFSET);
    if (!(reg_val & (1U << fifo_id)) && !(reg_val & (0x100U << fifo_id)))
      break;
    timeout--;
  }
...
static uint32_t can_fd_dlc_to_len(uint8_t dlc) {
  static const uint8_t dlc_to_len[16] = {0U, 1U,  2U,  3U,  4U,  5U,  6U,  7U,
...
enum can_error can_rx_fifo_setup(uint32_t base_addr, uint8_t fifo_id,
  ...
  size_val |=
      ((dc_size << CAN_RX_FQ_SIZE_DC_SIZE_POS) & CAN_RX_FQ_SIZE_DC_SIZE_MASK);
 ...
 * @desc_phys_addr: Physical address of descriptor linked list
 * @max_desc: Maximum number of descriptors
 * @dc_start_addr: Data container start address
 * @dc_size: Data container size
 *
 * Return: enum can_error code
 */
 ...
 * @base_addr: CAN controller base address
 * @fifo_id: RX FIFO queue index (0-7)
 * @has_msg: Pointer to receive result
 *
 * Return: enum can_error code
 */
 ...
 * @base_addr: CAN controller base address
 * @fifo_id: RX FIFO queue index (0-7)
 * @fill_level: Pointer to receive fill level
 *
 * Return: enum can_error code
 */
 ...
 * @base_addr: CAN controller base address
 * @fifo_id: RX FIFO queue index (0-7)
 * @new_addr: New read pointer address
 *
 * Return: enum can_error code
 */
...
/*
 * Interrupt handling, statistics, and utility functions
 */

/* PRT status register definitions */
...
/*
 * can_register_callbacks - Register interrupt callbacks
 * @base_addr: CAN controller base address
 * @callbacks: Pointer to callback configuration
 *
 * Return: enum can_error code
 */
enum can_error
can_register_callbacks(uint32_t base_addr,
...
/*
 * can_irq_handler - CAN Interrupt Handler
 * @base_addr: CAN controller base address
 *
...
void can_irq_handler(uint32_t base_addr) {
  ...
  if (g_callbacks_registered) {
    ...
    /* TX Abort */
    if (func_raw & CAN_IRC_FUNC_MH_TX_ABORT_IRQ_MASK) {
      g_can_stats.tx_error_frames++;
      if (g_can_callbacks.error_callback)
        g_can_callbacks.error_callback(CAN_ERROR_ARB_LOST,
                                       g_can_callbacks.user_ctx);
    }

    /* RX Abort */
    if (func_raw & CAN_IRC_FUNC_MH_RX_ABORT_IRQ_MASK) {
  ...
  if (err_raw) {
    if (err_raw & CAN_IRC_ERR_BUS_ERR_MASK) {
      if (g_callbacks_registered && g_can_callbacks.error_callback)
        g_can_callbacks.error_callback(CAN_ERROR_PROTOCOL,
                                       g_can_callbacks.user_ctx);
    }
    if (err_raw & CAN_IRC_ERR_ARB_LOST_MASK) {
      g_can_stats.arb_lost_count++;
      if (g_callbacks_registered && g_can_callbacks.error_callback)
        g_can_callbacks.error_callback(CAN_ERROR_ARB_LOST,
                                       g_can_callbacks.user_ctx);
    }
    if (err_raw & CAN_IRC_ERR_DMA_ERR_MASK) {
      if (g_callbacks_registered && g_can_callbacks.error_callback)
        g_can_callbacks.error_callback(CAN_ERROR_DMA, g_can_callbacks.user_ctx);
    }
  }

  /* Update error counters from PRT STAT */
  ...
  g_can_stats.error_warning = !!(prt_evnt & CAN_PRT_EVNT_E_WARN_MASK);
  g_can_stats.error_passive = !!(prt_evnt & CAN_PRT_EVNT_E_PASSIVE_MASK);
  g_can_stats.bus_off = !!(prt_evnt & CAN_PRT_EVNT_BUS_OFF_MASK);
  ...
  if (new_state != g_can_stats.bus_state) {
    ...
    if (g_callbacks_registered && g_can_callbacks.bus_state_callback)
      g_can_callbacks.bus_state_callback(new_state, g_can_callbacks.user_ctx);
  }
}

/*
 * can_get_irq_pending - Get pending interrupt status
 * @base_addr: CAN controller base address
 * @func_pending: Pointer to functional status (may be NULL)
 * @err_pending: Pointer to error status (may be NULL)
 * @safety_pending: Pointer to safety status (may be NULL)
 *
 * Return: enum can_error code
 */
...
/*
 * can_get_stats - Get CAN statistics and error counters
 * @base_addr: CAN controller base address
...
/*
 * can_get_bus_state - Get current bus state
 * @base_addr: CAN controller base address
 * @state: Pointer to receive bus state
 *
 * Return: enum can_error code
 */
enum can_error can_get_bus_state(uint32_t base_addr,
                                 enum can_bus_state *state) {
  uint32_t prt_evnt;
...
/*
 * can_clear_stats - Clear statistics counters
 * @base_addr: CAN controller base address
 *
 * Return: enum can_error code
 */
...
/*
 * can_set_loopback - Set loopback mode
 * @base_addr: CAN controller base address
...
/*
 * can_set_listen_only - Set listen-only mode
 * @base_addr: CAN controller base address
 * @enable: True to enable listen-only mode
 *
 * Return: enum can_error code
 */
...
/*
 * can_get_version - Get controller version information
 * @base_addr: CAN controller base address
 * @mh_version: Pointer to MH version (may be NULL)
 * @prt_version: Pointer to PRT version (may be NULL)
 *
 * Return: enum can_error code
 */
...
/*
 * can_software_reset - Perform software reset
 * @base_addr: CAN controller base address
 *
 * Return: enum can_error code
 */
